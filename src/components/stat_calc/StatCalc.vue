...
function calcSourceData(data){
  ...
  switch (data.job) {
    ...
    default:
      for (const s of jobs[data.job].ps){
        result.total_prim_stat += (data[s] * (1+data[s+'R']/100) + data[s+'D'])
      }
      
      for (const s of jobs[data.job].ss){
        result.total_second_stat += data[s] * (1+data[s+'R']/100) + data[s+'D']
      }
  ...
  //属性攻击力
  result.dDamage = (result.damage * (1+data.damR/100) * (1+data.pmdR/100))

  //一般伤害
  result.nDamage = (result.damage * (1+data.damR/100+data.ndR/100) * (1+data.pmdR/100))

  //BOSS怪物
  result.bDamage = (result.damage * (1+data.damR/100+data.bdR/100) * (1+data.pmdR/100))
  ...
  result.minSingleLineDamage = result.skillDamage * (1.2 + data.cdR/100) * weapon_mastery.value
...
class Buffs{
  ...
  constructor() {
    ...
    this.default = reactive([
      new Buff(`家族`,new BuffStat("bdR",30),new BuffStat("damR",30),new BuffStat("cdR",30)),
      new Buff(`Buff机`,new BuffStat("pmad",340),new BuffStat("bdR",80),new BuffStat("damR",10),new BuffStat("atR",10),new BuffStat("str",50),new BuffStat("int",50),new BuffStat("luk",50),new BuffStat("dex",50),new BuffStat("imdR",20),new BuffStat("imdR",15),new BuffStat("imdR",15)),
...
const handleAllChecked = (value) =>{
  buffs.default.forEach((buff)=>{
    buff.check = value;
  });
};

const updateAllChecked = () => {
  const allSelected = buffs.default.every((buff) => buff.check);
  const someSelected = buffs.default.some((buff) => buff.check);
  allChecked.value = allSelected ? true : someSelected ? false : false;
};

watch(
  () => buffs.default.map((buff) => buff.check),
  () => {
    updateAllChecked();
  },
  { deep: true }
...
function calcSourceAddonData(addons){
  ...
  //表功
  result.dDamage = formatFloat((diffBDamage / (1+buffData.damR/100+buffData.bdR/100) * (1+buffData.damR/100)))
  //傷害/b傷
  ...
  //st
  const diffSt = diffRDamage / (buffData.pmad * (1+buffData.pmadR/100) + buffData.pmadD)
  let aSt = 0
  switch (data.job) {
    case "3122":
  ...
  <n-page-header :title="currentStat.showName" @back="handleBack">
    <n-card>
      <n-tabs default-value="input" size="large" justify-content="space-evenly">
        <n-tab-pane name="input" tab="属性设定">
          <n-form>
            <n-collapse v-model:expanded-names="inputPanelCollapseExpanded">
              <n-collapse-item title="等级、职业" name="1">
              ...
              <n-collapse-item title="常規屬性" name="2">
                <n-space vertical>
                  <n-alert :show-icon="false">
                    仅开启怪怪和常驻BUFF，填写面板显示的值，若使用灵药请将火眼点掉
                  </n-alert>
                  <n-grid item-responsive responsive="screen" x-gap="12">
        ...
        <n-tab-pane name="buffs" tab="BUFF">
          <n-space vertical>
            <n-alert :show-icon="false">
              更适合黎明的BUFF计算器，涵盖黎明所有常用BUFF，其余BUFF请自行添加<br> 
              需要计算站街状态无视此页<br>
              家族技能无视根据技能等级情况自行添加至第一页<br>
              如需计算实战时面板(武公、塔戒、爆发技能、战斗中触发的LINK技能)，自行添加BUFF即可<br></br>
            </n-alert>
            ...
            <n-collapse v-model:expanded-names="resultPanelCollapseExpanded">
              <n-collapse-item title="机体" name="1">
                ...
                <n-grid item-responsive responsive="screen" x-gap="12">
                  <n-gi :span="gis">
                    <n-popover trigger="hover">
                      <template #trigger>
                        属性攻击力：{{numberFormat(currentStatCalcResult.dDamage)}}
                      </template>
                  ...
                  <n-gi :span="gis">
                    <n-popover trigger="hover">
                      <template #trigger>
                        BOSS怪物：{{numberFormat(currentStatCalcResult.bDamage)}}
                      </template>
                  ...
                  <n-gi :span="gis">
                    <n-popover trigger="hover">
                      <template #trigger>
                        一般怪物：{{numberFormat(currentStatCalcResult.nDamage)}}
                      </template>
                  ...
                  <n-gi :span="gis">
                    <n-popover trigger="hover">
                      <template #trigger>
                        无视防御：{{formatFloat(currentStatCalcResult.imdr)}}%
                      </template>
                  ...
                  <n-form-item-gi label-placement="left" label="武器熟练度%" :span="gis">
                    <n-input-number min="0" max="100" v-model:value="weapon_mastery">
                      <template #suffix>
                        %
                      </template>
                    </n-input-number>
              ...
              <n-collapse-item title="技能设定" name="3">
                <n-grid item-responsive responsive="screen" x-gap="12">
                  ...
                  <n-form-item-gi label="技能无视防御%增加" :span="gis">
                    <n-input-number min="0" v-model:value="skill_ied">
                      <template #suffix>
                        %
                      </template>
                    </n-input-number>
              ...
              <n-collapse-item title="极限属性" name="6">
                <n-space vertical>
                  <n-alert :show-icon="false">
              ...
              <n-collapse-item title="HEXA属性" name="7">
                <n-space vertical>
                  <n-alert>
                    <n-popover trigger="hover">
                      <template #trigger>
